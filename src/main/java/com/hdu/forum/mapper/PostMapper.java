...
public interface PostMapper {
    ...
    @Insert("INSERT INTO post(user_id, category_id, title, content, view_count, like_count, " +
            "comment_count, is_top, status, create_time, update_time) " +
            "VALUES(#{userId}, #{categoryId}, #{title}, #{content}, #{viewCount}, #{likeCount}, " +
            "#{commentCount}, #{isTop}, #{status}, #{createTime}, #{updateTime})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Post post);
    
    @Update("UPDATE post SET category_id=#{categoryId}, title=#{title}, content=#{content}, " +
            "is_top=#{isTop}, status=#{status}, update_time=#{updateTime} WHERE id=#{id}")
